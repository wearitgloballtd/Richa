                ...
                <Slider {...sliderSettings}>
                    {slides.map((slide) => (
                        <div key={slide.id} className="w-full focus:outline-none">
                            <div className={`flex flex-col lg:flex-row gap-8 lg:gap-16 items-center ${slideClassName}`}>
                                {/* Left Side: Image Container */}
                                <div className="w-full lg:w-1/2">
                                    <div className="w-full aspect-[4/3] relative">
                                        <div className={`w-full h-full relative overflow-hidden ${imageBorderRadius}`}>
                                            <img
                                                src={slide.image}
                                                alt={slide.title}
                                ...
                                <div className="w-full lg:w-1/2 text-left pl-0 lg:pl-8">
                                    <span
                                        className={`inline-block py-1 px-3 border border-black/20 rounded-full font-['FoundersGroteskCondensed'] tracking-wider text-sm mb-2 bg-white text-black ${tagClassName}`}
                                    >
                                        {slide.tag}
                                    </span>
                                    <h2 className={`${titleClassName} font-bold leading-tight mb-2 font-['FoundersGroteskCondensed'] text-black`}>
                                        {slide.title}
                                    </h2>
